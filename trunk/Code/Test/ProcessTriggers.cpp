...
#include "../PhysX/Trigger/TriggerManager.h"
...
#include "../PhysX/Trigger/Trigger.h"
...
CPhysicController* gController = 0;
CPhysicUserData* userdata = 0;

CProcessTriggers::CProcessTriggers()
{
  m_fSpeed1 = 5.0f;
  m_pPlayer1=NULL;
}
...
CProcessTriggers::~CProcessTriggers()
{
  CHECKED_DELETE(gController);
//  CHECKED_DELETE(userdata);
}

void CProcessTriggers::Init ()
{
...
// m_SceneEffectManager = CORE->GetSceneEffectManager();//new CSceneEffectManager();
  ...
  gController = new CPhysicController(2.0f, 2.0f, 0.5f, 0.1f, 0.5f, 1, userdata, Vect3f(0.f,1.0f,0.f)/*controller->GetPosition()*/,0.f);

  CORE->GetPhysicsManager()->AddPhysicController(gController);

  //init de triggers
  std::vector<CTrigger*> vectorTriggers = CORE->GetTriggerManager()->GetTriggersVector();

  std::vector<CTrigger*>::iterator l_iter = vectorTriggers.begin();
  for(;l_iter != vectorTriggers.end() ;++l_iter)
  {
    (*l_iter)->Init();
  }
...
void CProcessTriggers::RenderScene(CRenderManager* rm) 
{
  /*PhysX test*/
	//CORE->Render();
	Render3D(rm); 
...
void CProcessTriggers::Update(float elapsedTime)
{
  ...
  CORE->GetRenderableObjectsManager()->Update(elapsedTime);
	...
	//PhysXController
  gController->Move(Vect3f(0.0f,0.0f,0.0f),elapsedTime);
}


void CProcessTriggers::PreRender(CRenderManager* rm) 
{
//	m_SceneEffectManager->PreRender(rm,this);
}

void CProcessTriggers::Render(CRenderManager* rm) 
{
...
*/
  bool p = CORE->GetPhysicsManager()->GetDebugRenderMode();
...
void CProcessTriggers::LoadXMLs()
{
	//Load XML
  CCore * l_pCore = CORE;
  l_pCore->GetEffectManager()->Load("Data/Effects.xml");
  l_pCore->GetStaticMeshManager()->Load("Data/StaticMeshManager - escenarionew.xml");
  l_pCore->GetLigthManager()->Load("Data/lights.xml");
  l_pCore->GetAnimatedModelManager()->Load("Data/AnimatedModels.xml");
  l_pCore->GetRenderableObjectsManager()->Load("Data/RenderableObjects - escenarionew.xml");
...
void CProcessTriggers::Render3D(CRenderManager* rm)
{
	// render manager
...
void CProcessTriggers::Render2D(CRenderManager* rm)
{
}

void CProcessTriggers::RenderDebugInfo()
{
  CProcess::RenderDebugInfo();
}

void CProcessTriggers::CheckActionsFromInputs(float elapsedTime)
{
...
void CProcessTriggers::MovPlayer(float elapsedTime)
{
  ...
  CActionToInput* l_Action = CORE->GetInputManager()->GetActionToInput();
    ...
    gController->Move(l_VectorDirector, elapsedTime);
    l_CameraObject->SetPosition(gController->GetPosition());
    ...
    l_VectorDirector.y = 0.0f;
    l_VectorDirector.z = sin(l_ActualYaw+FLOAT_PI_VALUE/4);
    gController->Move((l_VectorDirector*-1), elapsedTime);
    l_CameraObject->SetPosition(gController->GetPosition());
		//m_pPlayer->SetPosition(l_vPos);
		//CORE->GetLigthManager()->GetResource("Spot01")->SetPosition(l_vPos);
	}
    ...
    gController->Move(l_VectorStrafe, elapsedTime);
    l_CameraObject->SetPosition(gController->GetPosition());
	}
	...
	{
    gController->Jump(1); //Testing purposes ony
  }
...
void CProcessTriggers::MovCamera()
{
  //CAMERAS
...
void CProcessTriggers::MovPlayerIA(float _elapsedtime)
{
...
//
//float CProcessTriggers::AjusteDireccion()
//{
...
void CProcessTriggers::RenderMovIA(CRenderManager *rm)
{
  //Vect3f l_V3PosPlayer1 = m_pPlayer1->GetPosition();
...
Vect3f CProcessTriggers::GetPositionObj(CObject3D * p_vObj)
{
...
void CProcessTriggers::InitCameras()
{
}

void CProcessTriggers::MovMouse()
{
...
void CProcessTriggers::UpdateLua()
{
	// Carga de script
...
void CProcessTriggers::MemoryTest()
{
