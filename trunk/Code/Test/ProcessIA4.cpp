...
#include "../Videogame/CoreFIH.h"
#include "../Videogame/Player/Player.h"
#include "../Videogame/Player/PlayerAnimData.h"
...
void CProcessIA4::Init ()
{
  ...
  //substitucion de player
  //Bots
	//m_pPlayer1 = (CRenderableAnimatedInstanceModel*)CORE->GetRenderableObjectsManager()->GetInstance("mi_amigo");
 // 
 // m_pPlayer1->SetPosition(Vect3f(m_pPlayer1->GetPosition().x,m_pPlayer1->GetPosition().y + 2.f,m_pPlayer1->GetPosition().z));
 // //crea controller
 ...
 // gController1 = new CPhysicController(0.5f, 1.0f, 0.5f, 0.1f, 0.01f, 1, userdata, m_pPlayer1->GetPosition()/*controller->GetPosition()*/);
 // CORE->GetPhysicsManager()->AddPhysicController(gController1);


  //gActor = new CPhysicActor(userdata);
  //gActor->AddBoxSphape(Vect3f(0.3f,3.0f,0.3f),m_pPlayer1->GetPosition());
  //CORE->GetPhysicsManager()->AddPhysicActor(gActor);
...
void CProcessIA4::Update(float elapsedTime)
{
...
//    gController1->Move(Vect3f(0.0f,0.0f,0.0f), elapsedTime);
//////  gController1->Move(Vect3f(0.00001f,0.0f,0.00001f), elapsedTime);
//  gActor->SetGlobalPosition(gController1->GetPosition());
//  m_pPlayer->Update(elapsedTime);  
}


void CProcessIA4::PreRender(CRenderManager* rm) 
{
...
void CProcessIA4::Render3D(CRenderManager* rm)
{
	// render manager
...
void CProcessIA4::Render2D(CRenderManager* rm)
{
}
...
void CProcessIA4::CheckActionsFromInputs(float elapsedTime)
{
...
void CProcessIA4::UpdateLua()
{
  ...
  if((m_DeltaFromXML = l_Action->DoAction("LoadLuaFile")) != 0.0f )
  {
    ...
    CORE->GetScriptManager()->LoadFile("./Data/LUAScripts/zombis.lua");
  }
  // Carga del script LUA
...
void CProcessIA4::MovPlayer(float elapsedTime)
{
...
void CProcessIA4::MovPlayerIA(float _elapsedtime)
{
...
//
//float CProcessIA4::AjusteDireccion()
//{
...
void CProcessIA4::RenderMovIA(CRenderManager *rm)
{
  //Vect3f l_V3PosPlayer1 = m_pPlayer1->GetPosition();
...
Vect3f CProcessIA4::GetPositionObj(CObject3D * p_vObj)
{
...
void CProcessIA4::InitCameras()
{
}

void CProcessIA4::MovMouse()
{
