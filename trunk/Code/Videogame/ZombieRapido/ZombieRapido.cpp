...
// Init data
//----------------------------------------------------------------------------
bool CZombieRapido::Init(const CXMLTreeNode &m)
{
  ...
  {
    m_fExtraSpeed =	m.GetFloatProperty("extra_speed");
  }
  return bIsOk;
}

bool CZombieRapido::Init(CZombie &z){
...
// Recalculate IA
//----------------------------------------------------------------------------
char CZombieRapido::RecalculateIA(Vect3f hisPosition)
{
...
// Update
//----------------------------------------------------------------------------
void CZombieRapido::Update(float elapsedTime)
{
...
//  CORE->GetAnimatedModelManager()->GetInstance(this->GetInstance()->GetName())->BlendCycle
  switch(GetState())
  { 
    case (char)IDLE:
    ...
    break;
    case (char)ATACANDO:
//      m_pRenderableAnimatedInstanceModel->GetAnimatedInstanceModel()->ExecuteAction(4,1.f);
      //PosicionaManoDerecha();
      //PosicionaManoIzquierda();
    break;
    case (char)RECIBIENDOGOLPE:
//      m_pRenderableAnimatedInstanceModel->GetAnimatedInstanceModel()->BlendCycle(2,elapsedTime);
    break;
    ...
    break;
    case (char)MURIENDO:
//      m_pRenderableAnimatedInstanceModel->GetAnimatedInstanceModel()->ExecuteAction(5,elapsedTime);
    break;
  }
}
...
// Render
//----------------------------------------------------------------------------
void CZombieRapido::Render(CRenderManager* rm)
{//TODO..
    Mat44f l_t;
	  l_t.SetIdentity();
    rm->SetTransform(l_t);
