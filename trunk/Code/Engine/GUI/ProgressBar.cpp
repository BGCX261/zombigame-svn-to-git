...
void CProgressBar::Render	(CRenderManager *renderManager, CFontManager* fm)
{
	if( CGuiElement::m_bIsVisible)
	{
		...
		{
			renderManager->DrawQuad2D(CGuiElement::m_Position,CGuiElement::m_uWidth,CGuiElement::m_uHeight, CRenderManager::UPPER_LEFT, m_pBackgroundTexture);
		}
		else
		{
			renderManager->DrawQuad2D(CGuiElement::m_Position,CGuiElement::m_uWidth,CGuiElement::m_uHeight, CRenderManager::UPPER_LEFT, m_BackGroundColor);
		}
		...
		{
			renderManager->DrawQuad2D(posProgress,w,h, CRenderManager::UPPER_LEFT, m_pProgressTexture);
		}
		else
		{
			renderManager->DrawQuad2D(posProgress,w,h, CRenderManager::UPPER_LEFT, m_ProgressColor);
		}
		Vect2i pos;
