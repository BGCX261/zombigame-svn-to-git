...
void CLogRender::Render (CRenderManager* renderManager, CFontManager* fm, CColor color)
{
	if( m_bIsVisible )
	{
      ...
      renderManager->GetWandH(win_w, win_h);
			if (w > win_w)
			{
	...
	{
		std::string myInfo;
    CORE->GetInputManager()->GetActionToInput()->GetActionConfig(myInfo, "Logger");

		std::string l_sInfo;
		baseUtils::FormatSrting(l_sInfo, "Press %s to view the Logger", myInfo.c_str());
...
void CLogRender::RenderLines (	CRenderManager* renderManager, CFontManager* fm, const std::vector<SLog>& vecLogs, 
